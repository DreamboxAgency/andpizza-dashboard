      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
        ...
        {summaries.length === 0 && (
          <div style={{
            ...
            <p style={{ fontFamily: 'var(--font-display)', fontSize: '1.4rem', letterSpacing: '0.06em', color: '#4A4A4A', marginBottom: 12 }}>
              NO DATA YET
            </p>
            <p style={{ color: '#4A4A4A', fontSize: '0.85rem', marginBottom: 24 }}>
              Upload a monthly report to get started with {client?.name}.
            </p>
            <a href={`/upload?client=${clientSlug}`} className="btn-primary" style={{ textDecoration: 'none', padding: '12px 24px' }}>
              Upload First Report →
            </a>
          </div>
        )}
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={spendByMonth}>
                    ...
                    <Tooltip
                      ...
                      formatter={(v: any) => [`${Number(v).toFixed(1)}%`, 'ROAS']}
                    />
              ...
              <div className="stat-card">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={spendChartData}>
                    ...
                    <Tooltip
                      contentStyle={{ background: '#1A1A1A', border: '1px solid #2E2E2E', borderRadius: 6 }}
                      formatter={(v: any, name: string) => [`$${Number(v).toLocaleString()}`, name.replace('_', ' ').toUpperCase()]}
                    />
